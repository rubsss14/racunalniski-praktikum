...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \(\text{k}_n\) označimo število
kršitev omejitve $n$, z \(\text{u}_n\) pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\[
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na $j$-tem koraku označimo s ??.
...
Naj bo $n=100$ ter \(\text{s}_1\) sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
...
se sprememba \(\text{s}_2\), ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
